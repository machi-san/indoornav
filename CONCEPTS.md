...
* **Cooperative shutdown** — a pattern where the main thread signals worker threads to stop (via a shared flag) rather than forcibly killing them. Each worker is responsible for noticing the signal and exiting its own loop cleanly at the next check. Contrasts with forced termination, where the main thread kills workers directly and risks leaving resources unreleased (open files, locked GPIO pins, unreleased camera handles). Cooperative shutdown is correct but not instant — a worker mid-iteration only notices the flag at the next check, introducing latency up to the duration of one loop cycle. In `main.py`, this is implemented as `shutdown_flag = True` set by the main thread on `KeyboardInterrupt`, with each worker checking `while not shutdown_flag:` at the top of its loop. The `daemon=True` flag on each thread acts as a safety net: even if a worker fails to notice the flag, the daemon flag ensures it dies when the main thread exits.
- **Rate limiting at the producer side for human-perceptual outputs** — when code produces output intended for human perception (speech, on-screen indicators, haptic feedback, audible alarms), the rate of generation should be bounded *at the producer*, not at the consumer. Producers can generate events faster than humans can perceive them: a 0.5-second sensor poll loop produces 2 events per second per sensor, but a spoken alert takes ~1-2 seconds to play, and a human needs additional time between alerts to actually process the information. Without producer-side limits, queues fill faster than they drain — leading to stale audio (the user hears alerts about objects that are no longer there), system unresponsiveness (consumer thread blocked on slow output, can't process shutdown signals), and a degraded user experience. The fix is per-class or per-event cooldowns at the producer: track the timestamp of the last emission, skip new emissions until a minimum interval has passed. In `vision.py`, this is implemented as `last_alert_times[class_name]` with a 3-second `ALERT_COOLDOWN`; in `ultrasonic.py`, the same pattern applies per (sensor, alert level) pair. The principle generalises beyond audio: any output that intersects human perception benefits from producer-side rate limiting.
\---
...
* **Cleanup that never runs** — a subtle failure mode where cleanup code is written but unreachable. In v1's `ultrasonic_loop()`, cleanup was placed in an `except KeyboardInterrupt:` block, but `KeyboardInterrupt` is raised in the main thread (where the `while True: time.sleep(1)` loop catches it), not in worker threads. The ultrasonic loop also did not check `shutdown_flag`, so neither path triggered `cleanup()`. The thread terminated cleanly only because `daemon=True` kills it when the main process exits, but GPIO pins were left for the OS to reclaim rather than being explicitly released. The fix: cooperative shutdown via `shutdown_flag` in the loop condition, with `GPIO.cleanup()` in a `try/finally` block (mirroring the camera thread's pattern). Lesson — exception handlers only catch exceptions that actually reach them; if you depend on a handler running, verify the exception can get there.
